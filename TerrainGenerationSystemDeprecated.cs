...
namespace TechnOllieG
{
	...
	public class TerrainGenerationSystem : JobComponentSystem
	{
		...
		private bool _currentlyGeneratingChunks = false;
		private JobHandle _chunkGenerationHandle = default;
		private List<int2> _chunksScheduledForGeneration = new List<int2>();
		private NativeMultiHashMap<int2, int3> _blockPositions;
		...
		protected override void OnStartRunning()
		{
			...
			_blockPositions = new NativeMultiHashMap<int2, int3>((_data.renderDistance * 2 + 1) * 2 * 256 * 3, Allocator.Persistent);
		}

		protected override void OnDestroy()
		{
			_blockPositions.Dispose();
		}
		...
		protected override JobHandle OnUpdate(JobHandle inputDeps)
		{
			...
			GenerateChunks(ref inputDeps);
			//CalculateExposedQuads(ref inputDeps);
			DegenerateChunks();

			if (_currentlyGeneratingChunks && _chunkGenerationHandle.IsCompleted)
			{
				_chunksScheduledForGeneration.Clear();
				_currentlyGeneratingChunks = false;
			}
		...
		private void CalculateExposedQuads(ref JobHandle dependency)
		{
			foreach(int2 chunk in _chunksScheduledForGeneration)
			{
				ChunkCoordinate coordinate = new ChunkCoordinate {chunkCoordinate = chunk};

				dependency = Entities.WithSharedComponentFilter(coordinate).ForEach(
		...
		private void GenerateChunks(ref JobHandle dependency)
		{
			if (_chunksToGenerate.Count == 0)
				return;

			for (int i = 0; i < _chunksToGenerate.Count; i++)
			{
				int2 chunk = _chunksToGenerate[i];
				GenerateChunk(chunk, ref dependency);

				_chunksScheduledForGeneration.Add(chunk);
			}

			_chunkGenerationHandle = dependency;
			_currentlyGeneratingChunks = true;
		...
		private void DegenerateChunks()
		{
			...
			foreach (int2 chunk in _chunksToDegenerate)
			{
				...
				_blockPositions.Remove(chunk);
			}
			
			_chunksToDegenerate.Clear();
		}

		private void GenerateChunk(int2 chunkCoordinates, ref JobHandle dependency)
		{
			...
			var blockPositions = _blockPositions.AsParallelWriter();

			dependency = Entities.WithSharedComponentFilter(coordinate)
				.ForEach((int entityInQueryIndex, ref Translation translation, in BlockState state) =>
				{
					...
					float noiseX = (float) (zeroIndexedChunkCoordinate.x * chunkSize + chunkSpaceBlockCoordinate.x) /
						worldSizeInBlocks * scale + xOffset;
					float noiseY = (float) (zeroIndexedChunkCoordinate.y * chunkSize + chunkSpaceBlockCoordinate.y) /
						worldSizeInBlocks * scale + yOffset;
					...
					//blockPositions.Add(chunkCoordinates, blockPos);

					translation.Value = blockPos;
				}).Schedule(dependency);
			
			var rigidbodySystem = World.GetExistingSystem<SmoothRigidBodiesGraphicalMotion>();
			rigidbodySystem.AddInputDependency(dependency);
			dependency.Complete();
